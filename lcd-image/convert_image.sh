...
stream -map rgb -storage-type char $1 - | \
  perl -0777 -ne 'while ($pixel = substr($_, 0, 3, "")) { $red = ord(substr($pixel, 0, 1)); $green = ord(substr($pixel, 1, 1)); $blue = ord(substr($pixel, 2, 1)); $color = (($red >> 3) << 11) + (($green >> 2) << 5) + ($blue >> 3); print chr($color >> 8); print chr($color & 0xff) }' \
  > $2
